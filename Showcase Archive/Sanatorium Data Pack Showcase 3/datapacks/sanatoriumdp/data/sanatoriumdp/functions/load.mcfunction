##start the sounds!
schedule function sanatoriumdp:soundlocation1a 70s
schedule function sanatoriumdp:footsteps1a 75s
schedule function sanatoriumdp:footsteps1b 90s
schedule function sanatoriumdp:footsteps1c 50s
schedule function sanatoriumdp:bang1a 100s
schedule function sanatoriumdp:bang1b 130s
schedule function sanatoriumdp:paranormal1a 165s
schedule function sanatoriumdp:paranormal1b 60s
schedule function sanatoriumdp:soundlocation1b 115s
schedule function sanatoriumdp:soundlocation2a 85s
schedule function sanatoriumdp:soundlocation3a 55s

##debug test
say Sanatorium Map Edition Started!